...
void draw_setup_screen(void)
{
    static const char* labels[] = {"PDL SWAP", "SIDETONE"};
    bool values[] = {paddle_swap, sidetone_enabled};
    ...
    for (int i = 0; i < 2; i++) {
        // カーソル
        ssd1306_drawstr_sz(0, 14 + i * 14, (char*)(setup_cursor == i ? ">" : " "), 1, fontsize_8x8);
        ssd1306_drawstr_sz(10, 14 + i * 14, (char*)labels[i], 1, fontsize_8x8);
        ssd1306_drawstr_sz(90, 14 + i * 14, (char*)(values[i] ? " ON" : "OFF"), 1, fontsize_8x8);
    }

    // ボタンガイド
    ssd1306_fillRect(3, 48, 27, 8, 1);
    ssd1306_fillRect(35, 48, 27, 8, 1);
    ssd1306_fillRect(67, 48, 27, 8, 1);
    ssd1306_fillRect(99, 48, 27, 8, 1);

    const char arrow_up[]   = { 0x04, '\0' };
    const char arrow_down[] = { 0x01, '\0' };
    // F1:↑  F2:↓  F3:CHG  F4:SAVE
    ssd1306_drawstr_sz(12,   48, (char*)arrow_up,   0, fontsize_8x8);
    ssd1306_drawstr_sz(44,  48, (char*)arrow_down, 0, fontsize_8x8);
    ssd1306_drawstr_sz(71,  48, "CHG", 0, fontsize_8x8);
    ssd1306_drawstr_sz(100,  48, "SAV", 0, fontsize_8x8);
...
void handle_setup_mode(void)
{
    ...
    {
        SW_CLEAR();
        if (setup_cursor > 0) setup_cursor--;
        draw_setup_screen();
        return;
    }
    ...
    {
        SW_CLEAR();
        if (setup_cursor < 1) setup_cursor++;
        draw_setup_screen();
        return;
    }
    ...
    {
        SW_CLEAR();
        if (setup_cursor == 0) paddle_swap      = !paddle_swap;
        else                   sidetone_enabled = !sidetone_enabled;
        draw_setup_screen();
        return;
    }
