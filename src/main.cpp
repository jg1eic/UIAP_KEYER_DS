...
#define SW_PUSH_TH 5    // 押下判定（5ms × 5 = 25ms）
#define SW_1 (1 << 3) 
#define SW_2 (1 << 2)
#define SW_3 (1 << 1)
#define SW_4 (1 << 0)
...
uint8_t job_auto()
{
    ...
    if (e == '.')
        auto_squeeze = SQZ_DOT; // 1dit ON + 1dit OFF
    else
        auto_squeeze = SQZ_DASH; // 3dit ON + 1dit OFF
    elem++;
...
void update_speed_from_adc()
{
    ...
    dit_est = (1200UL * 1000)/(wpm * 256); // 1200ms / wpm を 256分周で表現
    key_spd = dit_est;  // = (1200/wpm) /0.256  4687.5 -> 4687

    char buf[16];
    sprintf(buf, "%2d", wpm);
    ssd1306_drawstr_sz(96, 0, buf, 1, fontsize_8x8);
    // TODO: スピードが変わったときだけリフレッシュするようにする
    ssd1306_refresh();
}
